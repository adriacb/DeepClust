...
class Logger:
	def __init__(self, filename, level='WARNING'):
...
def min_max_bycol(column):
    return (column - column.min()) / (column.max() - column.min())

def prepareInput3(df: pd.DataFrame):
    """
    Description:
    -----------
    ...
    normalization_layer.adapt(dfcp)

    return normalization_layer(df.to_numpy())
...
def prepareInput(df: pd.DataFrame):
    ...
    return tf.convert_to_tensor(scaled, dtype=tf.float64)
...
def prepareInput2(df: pd.DataFrame):
    """
    Description:
    -----------
        Creates the input for the neural network.
